...
const questoesAleatorias = embaralhar(questoes);
const dezQuestoesAleatorias = questoesAleatorias.slice(0, 10);
let acertos = 0;
let mensagemFinal = "";
let nome = "";

// função que recebe um array e retorna esse array embaralhado:
...
function iniciarJogo(){
    ...
    nome = entradaDados.question('Digite seu nome: ');

    for(let i = 0; i < dezQuestoesAleatorias.length; i++){
        console.log(`\n${i+1}: ${questoesAleatorias[i].pergunta}`);
        let resposta = entradaDados.question("Digite a resposta (ano): ");
    
        if(resposta == dezQuestoesAleatorias[i].resposta){
    ...
    if(acertos <= 3){
        mensagemFinal = "OH NÃO! Tente mais uma vez.";
    }else if(acertos > 3 && acertos <=6){
        mensagemFinal = "BOM TRABALHO! Pratique um pouco mais.";
    }else if(acertos > 6 && acertos <= 9){
        mensagemFinal = "MUITO BOM! Você acertou a maioria."
    }else{
        mensagemFinal = "EXCELENTE! Você é um verdadeiro expert.";
    }
    
    console.log("\nJogador(a): " + nome);
    console.log("Pontuação final: " + acertos + " acertos");
